...
/// supplied endpoints, a persisted good-peers JSON file, and DNS seeds
/// resolved with getaddrinfo. A monitor task prunes dead connections and
/// connects replacements. Deliberately simple: no scoring buckets, no addr
/// gossip — misbehaving peers are dropped and replaced.
    ...
    private func replenish() async {
        ...
        for endpoint in candidates(excluding: connected) where needed > 0 {
            ...
            do {
                ...
                continue // unreachable or handshake failed — try the next candidate
            }
        }
    }

    /// Manual peers first, then persisted good peers, then DNS seeds.
    private func candidates(excluding connected: Set<PeerEndpoint>) -> [PeerEndpoint] {
        var ordered = manualPeers + knownGood.subtracting(manualPeers)
        if ordered.filter({ !connected.contains($0) }).count < peerCount {
